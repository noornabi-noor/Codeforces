...
vector<int> fact1(N + 1);
vector<int> inv_fact(N + 1);

void precompute() {
    fact1[0] = 1;
    for (int i = 1; i <= N; i++) {
        fact1[i] = (1LL * fact1[i - 1] * i) % mod;
    }
    
    // inv_fact[N] = pwr(fact1[N], mod - 2);
    // for (int i = N - 1; i >= 0; i--) {
...
void solve(){
    ...
    // int n_inv=pwr(n,mod-2);

    // int k=mod_mul(s,n_inv);
    ...
    int n_mk=mod_sub(m,k);

    int n_pw=pwr(n_n,n_mk);

    int ans=mod_mul(n_s,n_pw);
