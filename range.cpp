...
void build(int SI,int l, int r){
    ...
    int mid=(l+r)/2;
    build(2*SI,l,mid);
    build((2*SI)+1LL,mid+1LL,r);
    ST[SI] =  ST[2*SI] + ST[(2*SI)+1LL];
}

int query(int SI, int rL, int rR, int l , int r){
    if(rR<l || rL>r){
        return 0LL;
    }
    if(rL>=l && rR<=r){
    ...
    int mid=(rL+rR)/2;
    int Leftn=query(2*SI,rL,mid,l,r);
    int Rightn=query((2*SI)+1LL,mid+1LL,rR,l,r);
    return Leftn+Rightn;
}

void update(int SI, int start, int end, int pos, int val){
    if(start==end){
        ST[SI]+=val;
    }
    else{
        ...
        if(start<=pos && pos<=mid){
            update(SI*2,start,mid,pos,val);
        }
        else{
            update((SI*2)+1,mid+1,end,pos,val);
        }
        ST[SI]=(ST[2*SI]+ST[(2*SI)+1LL]);
    }
}
...
void solve(){
    ...
    while(q--){
        ...
        if(type==1){
            ...
            update(1,1,num,a,u);
            if(b!=num){
                update(1,1,num,b+1,-u);
            }
        }
        else{
            cin>>idx;
            int ans=query(1, 1, num, 1, idx);
            cout << ans << '\n';
