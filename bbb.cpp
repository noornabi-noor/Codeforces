...
void solve(){
    ...
    unordered_map<int, pair<int, int>> xorPairs;
    bool found = false;

    for (int i = 0; i < n && !found; i++) {
        for (int j = i + 1; j < n; j++) {
            int currentXor = v[i] ^ v[j];
            
            if (xorPairs.find(currentXor) != xorPairs.end()) {
                int x = xorPairs[currentXor].first;
                int y = xorPairs[currentXor].second;
                ...
                xorPairs[currentXor] = make_pair(i, j);
            }
        }
    }
