...
void solve(){
    ...
    while(q--){
        ...
        int sub1=mod_sub(j2,j1);
        int add1=mod_add(sub1,1);

        int pw1=pwr(3,add1);
        int sub2=mod_sub(pw1,1);

        int pw2=pwr(3,j1);
        int pw3=pwr(2,i1);
        int mul1=mod_mul(pw2,sub2);

        int mul2=mod_mul(mul1,pw3);

        int inv1=pwr(2,mod-2);
        int mul3=mod_mul(mul2,inv1);

        int r=mod_sub(i2,i1);
        int nr=mod_add(r,1);
        int pw4=pwr(2,nr);
        int sub3=mod_sub(pw4,1);

        int mul4=mod_mul(mul3,sub3);

        //int ans=mod_mul(mul4,sub3);
        cout<<mul4<<"\n";
    }
