...
// void preorderRecursive(const vll& tree, int index, int size) {
//     if (index > size || tree[index] == -1) return;  

//     cout << tree[index] << "\n"; 
    
//     preorderRecursive(tree, 2 * index, size); 
//     preorderRecursive(tree, 2 * index + 1, size); 
// }
...
// Function to recursively construct the tree and store the preorder traversal
void constructTree(int start, int end, vector<int> &result) {
    if (start > end) return;
    
    // Middle element is chosen as the root for balance
    int mid = (start + end + 1) / 2;
     // Visit the root node (preorder)

   
    constructTree(start, mid - 1, result);
    
    constructTree(mid + 1, end, result);
...
void solve() {
    ...
    int numNodes = (1 << N) - 1;  // Total number of nodes in the tree (2^N - 1)
    vector<int> result;           // To store the preorder traversal result

    // Construct the tree and fill the result with preorder traversal
    constructTree(1, numNodes, result);
