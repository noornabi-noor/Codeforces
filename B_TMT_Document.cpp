...
void solve(){
        ...
        int tc = 0,mc = 0;
        for (int i = 0;i<n;i++){
            if (s[i]=='M'){mpos.emplace_back(i);}
            else{tpos.emplace_back(i);}
        }
        for (char c:s){if (c=='T'){tc++;}else{mc++;}}
        if (n%3){cout << "NO\n";}
        else if (tc != 2*mc){cout << "NO\n";}
        else{
            bool pos = true;
            for (int i = 0;i<mc;i++){
                if (mpos[i] < tpos[i] or mpos[i] > tpos[i+mc]){pos = false;}
            }
            if (pos){cout << "YES\n";}
            else {cout << "NO\n";}
        }
