...
//int pwr(ld a,ld b) {a %= mod; int res = 1; while (b > 0) {if (b & 1LL) res = res * a % mod; a = a * a % mod; b >>= 1LL;} return res;}
...
void solve() {
    ...
    cin >> n;
    int p1 = pwr1(2LL, n)-2;
    // int p2 = mod_sub(p1,2LL);
    // int p3 = pwr(4LL, p2);
    // int p4=mod_mul(p3,6LL);
    cout << (6*pwr_mod(4,p1,mod))%mod;
}
