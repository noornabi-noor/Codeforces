...
void solve(){
    ...
    for (int i = 0; i < m; ++i) {
        cin >> ind[i];
        //ind[i]--; // converting 1-based index to 0-based index
    }
    
    string c;
    cin >> c;
    
    // Map to store the smallest possible character for each index
    map<int, char> updates;
    for (int i = 0; i < m; ++i) {
        int index = ind[i];
        char new_char = c[i];
        if (updates.count(index) == 0 || new_char < updates[index]) {
            updates[index] = new_char;
        }
    }
