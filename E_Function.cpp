...
int query(int SI, int rL, int rR, int l , int r,int v){
    if(rR<l || rL>r){
        return LLONG_MAX;
    }
    if(rL>=l && rR<=r){
    ...
    int mid=(rL+rR)/2;
    int Leftn=query(2*SI,rL,mid,l,r,v);
    int Rightn=query((2*SI)+1LL,mid+1LL,rR,l,r,v);
...
void solve(){
    ...
    while(q--){
        int l,r;
        cin>>l>>r;
        cout<<query(1,1,n,r-l+1,r,l-r)+a[r]<<"\n";
    }
