...
void solve(){
        ...
        for (int i = 1; i < n; i++) {
            if(abs(cities[i] - cities[i-1])>abs(cities[i] - cities[i+1]))
                ...
                sum[i] = sum[i-1] + abs(cities[i] - cities[i+1]);
        }
        for (int i = n-1; i >0 ; i--) {
            if(abs(cities[i+1] - cities[i])<abs(cities[i+2] - cities[i+1]))
                Sum[i] = Sum[i+1] + 1;
            else
                Sum[i] = Sum[i+1] + abs(cities[i+1] - cities[i]);
        }
        int m;
        cin>>m;
        while(m--){
            ...
            cin>>f>>t;
            if(f<=t){
                cout<<sum[t-1]-sum[f-1]<<endl;
            }else{
                cout<<Sum[t]-Sum[f]<<endl;
            }
        }
}
